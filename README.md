...
The Rakefile is the source of truth for project checks. To run the same build
checks used by CI from the root of the source code:

    bundle exec rake ci:build

For a narrower local loop, run individual tasks:

    bundle exec rake validate
    bundle exec rake lint
    bundle exec rake spec
    bundle exec rake docs:check
    bundle exec rake generated_manifest:check
    bundle exec rake security:bundle_audit
    bundle exec rake module:install_check

To run the Litmus acceptance tests locally, ensure Docker is running, then use:

    bundle exec rake ci:acceptance
    bundle exec rake ci:acceptance_teardown

### Release

This module uses Puppet Blacksmith to publish to the Puppet Forge.

Before releasing, run the release consistency checks:
...
On [occasion](https://github.com/alex-harvey-z3q/puppet-firewall_multi/issues/19), users of this module have reported confusing failure messages like:
